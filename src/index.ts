...
const args = String(process.env.npm_lifecycle_script).split(' ');

if (args.length < 2) {
    console.error('Please specify the database name');
}

const DIR_OUT = 'output';
const DATABASE = args[2].replaceAll('"', '');
...
async function main() {
    ...
    console.log((tables as any[]).map((table: any) => table.TABLE_NAME));
    ...
    for (const table of tables as any[]) {
        ...
        createFile(
            table['TABLE_NAME'],
            createType(table['TABLE_NAME'], columns),
        );
    }
}
